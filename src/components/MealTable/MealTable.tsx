  ...
  const items = props.products.map((item, index) => {
    ...
    return (
      <div className={styles.foodList__item__row}>
        ...
        {props.onDeleteClick ? (
          <button
            className={styles.foodList__item__product}
            value={index}
            onClick={(event: any) => props.onDeleteClick(event.target.value)}
          >
            Удалить
          </button>
        ) : (
    ...
    <div className={styles.foodList__item}>
      ...
      <div className={styles.foodList__item__grid}>
        <div className={styles.foodList__item__row}>
          ...
          {props.deletable ? (
            <div
              className={`${styles.foodList__item__header} ${styles.hidden}`}
            ></div>
          ) : (
            ""
          )}
        ...
        <div className={styles.foodList__item__row}>
          ...
          <div className={styles.foodList__item__result}>
            {finalCalories.toFixed(2)}
          </div>
          <div className={styles.foodList__item__result}>
            {finalPropteins.toFixed(2)}
          </div>
          <div className={styles.foodList__item__result}>
            {finalFats.toFixed(2)}
          </div>
          <div className={styles.foodList__item__result}>
            {finalCarbos.toFixed(2)}
          </div>
          {props.deletable ? (
            <div
              className={`${styles.foodList__item__result} ${styles.hidden}`}
            ></div>
          ) : (
            ""
          )}
      ...
      {props.onEditMeal ? (
        <div className={styles.foodList__ref}>
          <data value={props.index} onClick={onEditMeal}>
            редактировать
          </data>{" "}
        </div>
      ) : (
